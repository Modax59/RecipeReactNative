...
const validationSchema = Yup.object().shape({
    ...
    preparingTime: Yup.number().required().min(1).max(10000).label("Temps de préparation"),
...
function RecipeCreateScreen() {
    ...
    const handleSubmit = async (recipe, {resetForm}) => {
        ...
        if (result.ok) {
            const {image} = recipe
            const resultImage = await recipes.addImageRecipe(image, result.data['id'], (progress) => setProgress(progress));
            if (!resultImage.ok) {
    ...
    return (
        <Screen style={styles.container}>
            ...
            >
                ...
                <AppFormField maxLength={255} name="name" placeholder="Nom de la recette"/>
                <AppFormField
